...
void MemoryTest(void)
{
    ...
    unsigned int Start, End, TestVar;
    const unsigned int TestValue1 = 0x12345678, TestValue2 = 0x87654321;
    char c ;
    ...
    while (Start <= End) {
        ...
        *RamPtr = TestValue1;
        TestVar = *RamPtr;
        if (TestVar - TestValue1 != 0) {
            printf("\r\nMemory error!");
            printf("\r\nExpected %u, received %u", TestValue1, TestVar);
            return;
        }
        *RamPtr = TestValue2;
        TestVar = *RamPtr;
        if (TestVar - TestValue2 != 0) {
            printf("\r\nMemory error!");
            printf("\r\nExpected %u, received %u", TestValue2, TestVar);
            return;
        }
        Start += sizeof(Start);
    }
