...
void CanBus0_Receive(void)
{
    ...
    if (Can0_StatusReg & RBS_Bit)
    {
        data_in = Can0_RxBuffer1;
        printf("Received %02x", data_in);
    }
...
void CanBusTest(void)
{
    ...
    while(1)    {
        ...
        CanBus0_Transmit(0x00) ;       // transmit a message via Controller 0
